        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title mb-0">Role Form</h6>
                </div>
                <div class="card-body">
                    @if(!empty($data))
                        <form action="{{ route('role.update', $data->id) }}" method="post" id="clientForm">
                            @csrf
                            @method('PUT')
                            @else
                                <form action="{{ route('role.store') }}" method="post" id="clientForm">
                                    @csrf
                                    @endif
                                    <div class="row g-2">
                                        <div class="col-md-6">
                                            <label class="form-label">Name Role <span
